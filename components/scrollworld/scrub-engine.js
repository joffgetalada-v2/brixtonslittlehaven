...
function mountScrollWorld(container, config) {
  ...
  // LOCAL PATCH: the world can sit below in-flow chrome (sticky site header), so all
  // segment math anchors to the track's document offset instead of assuming scrollY 0.
  let trackTop = 0;
  ...
  function jumpTo(i) {
    ...
    window.scrollTo({ top: trackTop + seg.start + (seg.end - seg.start) * 0.5, behavior: reduce ? 'auto' : 'smooth' });
  }
  ...
  function read() {
    const y = (window.scrollY || window.pageYOffset) - trackTop;
    ...
    // LOCAL PATCH: when the world is embedded above normal page content, dissolve the
    // fixed overlay (sky/stage/copy/route/chrome) as the visitor scrolls past the last
    // segment, so the rest of the page takes over cleanly instead of ghost UI floating.
    const overshoot = y - totalW * vh;
    const exit = overshoot <= 0 ? 1 : smooth(clamp(1 - overshoot / (0.7 * vh)));
    [sky, stage, copylayer, route, topbar, scrollbar].forEach(n => {
...
// ESM export for Next.js bundling (the typeof guards above keep CJS/global intact).
